...
test('setup explains Node-only PlantUML and rejects retired Java installation flag', async () => {
  ...
  assert.match(stdout, /No flags installs authoring npm dependencies only \(no browser\)/);
  assert.match(stdout, /development\/browser tests only/);
...
test('doctor needs no Java or browser and reports the pinned JS engine', async () => {
  const { stdout } = await execute(process.execPath, [path.join(toolkitRoot, 'scripts/document.mjs'), 'doctor'], {
    env: { ...process.env, JAVA_BIN: '/missing/forbidden-java', PLANTUML_JAR: '/missing/forbidden.jar', BPMN_CHROMIUM_EXECUTABLE: '/missing/forbidden-browser', PLAYWRIGHT_BROWSERS_PATH: '/missing/forbidden-cache' }
  });
  ...
  assert.ok(!report.checks.some(check => /^(Java|JRE|PlantUML jar|playwright|Playwright Chromium)/.test(check.tool)));
});
